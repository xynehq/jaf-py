...
class AgentResponse:
    ...
    @classmethod
    def from_jaf_result(
        cls, result: Any, execution_time: Optional[float] = None
    ) -> "AgentResponse":
        ...
        if hasattr(result, "final_state") and hasattr(result.final_state, "messages"):
            ...
            if messages and len(messages) > 0:
                content = (
                    ...
                    else Message(role="assistant", content=str(messages[-1]))
                )
            else:
...
class RunnerCallbacks(Protocol):
    ...
    async def on_start(
        self, context: RunContext, message: Message, session_state: Dict[str, Any]
    ) -> None:
    ...
    async def on_after_llm_call(
        self, response: Message, session_state: Dict[str, Any]
    ) -> Optional[Message]:
        """
    ...
    async def on_before_tool_selection(
        self, tools: List[Tool], context_data: List[Any]
    ...
    async def on_after_tool_execution(
        self, tool: Tool, result: Any, error: Optional[Exception] = None
    ) -> Optional[Any]:
        """
    ...
    async def on_iteration_complete(
        self, iteration: int, has_tool_calls: bool
    ...
    async def on_check_synthesis(
        self, session_state: Dict[str, Any], context_data: List[Any]
    ...
    async def on_loop_detection(
        self, tool_history: List[Dict[str, Any]], current_tool: str
    ) -> bool:
        """
    ...
    async def on_fallback_required(self, context_data: List[Any]) -> Optional[FallbackCheckResult]:
        """
    ...
    async def on_context_update(
        self, current_context: List[Any], new_items: List[Any]
    ) -> Optional[List[Any]]:
        """
    ...
    async def on_excluded_ids_update(
        self, excluded_ids: List[str], new_ids: List[str]
    ) -> Optional[List[str]]:
        """
