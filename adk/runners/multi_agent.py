...
async def execute_multi_agent(
    ...
    try:
        ...
        execution_time = (time.time() - start_time) * 1000
        error_message = Message(role="assistant", content=f"Multi-agent execution failed: {str(e)}")
...
def merge_parallel_responses(
    responses: List[AgentResponse], config: MultiAgentConfig
) -> AgentResponse:
    """
...
def extract_keywords(text: str, config: Optional[KeywordExtractionConfig] = None) -> List[str]:
    """
    ...
    keywords = []
    for word in words:
        if len(word) >= config.min_word_length and word not in stop_words and not word.isdigit():
    ...
    for i, response in enumerate(responses):
        if isinstance(response, Exception):
            ...
            error_content = Message(
                role="assistant", content=f"Agent {agent_configs[i].name} failed: {str(response)}"
            )
            error_response = AgentResponse(
                content=error_content, session_state=session_state, artifacts={}
            )
