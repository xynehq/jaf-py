...
class TestAgentSelection:
    ...
    def test_context_preferences(self, sample_agents):
        ...
        context = RunContext(
            user_id="test",
            preferences={"preferred_agents": ["MathAgent"]}
        )
...
class TestDelegationDecisionExtraction:
    ...
    def test_text_pattern_delegation(self):
        ...
        response = AgentResponse(
            content=Message(role="assistant", content="I need to delegate to WeatherAgent for this request"),
            session_state={},
            artifacts={}
        )
    ...
    def test_artifact_delegation(self):
        ...
        response = AgentResponse(
            ...
            session_state={},
            artifacts={
                "delegation": {"target_agent": "NewsAgent"}
            }
        )
    ...
    def test_metadata_delegation(self):
        ...
        response = AgentResponse(
            ...
            artifacts={},
            metadata={
                "delegation": {"target_agent": "MathAgent"}
            }
        )
...
class TestMultiAgentExecution:
    ...
    async def test_conditional_strategy_execution(self, sample_agents, sample_context):
        ...
        with patch('adk.runners.multi_agent.jaf_run') as mock_run:
            mock_result = Mock()
            mock_result.final_state.messages = [Message(role="assistant", content="Weather is sunny")]
            mock_run.return_value = mock_result
            
            message = Message(role="user", content="What's the weather like?")
            
            result = await execute_multi_agent(
                config, {}, message, sample_context, mock_provider
            )
    ...
    async def test_sequential_strategy_execution(self, sample_agents, sample_context):
        ...
        with patch('adk.runners.multi_agent.jaf_run') as mock_run:
            ...
            responses = [
                Mock(final_state=Mock(messages=[Message(role="assistant", content="Weather processed")])),
                Mock(final_state=Mock(messages=[Message(role="assistant", content="News processed")]))
            ]
    ...
    async def test_hierarchical_strategy_execution(self, sample_agents, sample_context):
        ...
        coordinator = AgentConfig(
            name="Coordinator",
            instruction="I coordinate requests to other agents",
            tools=[]
        )
