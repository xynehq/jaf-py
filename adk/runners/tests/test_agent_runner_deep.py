...
from adk.runners.types import RunnerConfig, RunnerCallbacks, AgentResponse, RunContext
...
class MockTool:
    def __init__(self, name="search_tool", description="A mock search tool"):
        ...
        self._schema = ToolSchema(name=name, description=description, parameters=SearchToolArgs)
...
@pytest.fixture
def mock_agent():
    return Agent(name="TestAgent", instructions="You are a helpful test agent.", tools=[MockTool()])
...
def mock_jaf_run():
    ...
    with patch("adk.runners.agent_runner.jaf_run") as mock_run:
        ...
        default_message = Message(
            role="assistant", content="LLM response with tool call", tool_calls=[mock_tool_call]
        )
...
def base_config(mock_agent, mock_model_provider):
    return RunnerConfig(agent=mock_agent, session_provider=AsyncMock(), max_llm_calls=3)
...
class TestAdvancedControlFlow:
    async def test_on_before_llm_call_skips_llm_and_returns_custom_response(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
    ...
    async def test_iteration_forced_to_continue_by_callback(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
        ...
        first_message = Message(role="assistant", content="First response", tool_calls=[])

        second_message = Message(role="assistant", content="Final response", tool_calls=[])
    ...
    async def test_tool_execution_parameter_modification(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
        ...
        def mock_jaf_with_events(run_state, run_config):
            ...
            if run_config.on_event:
                ...
                start_event.data = {"tool_name": "search_tool", "args": {"query": "test"}}
...
class TestErrorAndEdgeCaseHandling:
    async def test_error_in_one_callback_does_not_stop_others(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
    ...
    async def test_no_tools_available_for_tool_call(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
        ...
        message_with_bad_tool = Message(
            role="assistant", content="Tool call", tool_calls=[mock_tool_call]
        )
...
class TestAgentHooks:
    async def test_on_query_rewrite_modifies_query(
        ...
        simple_message = Message(role="assistant", content="LLM response", tool_calls=[])
    ...
    async def test_on_check_synthesis_completes_early(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
        ...
        synthesis_message = Message(
            role="assistant", content="Synthesized final answer", tool_calls=[]
        )
    ...
    async def test_on_context_update_filters_context(
        ...
        def mock_jaf_with_context(run_state, run_config):
            ...
            mock_final_state.messages = [
                Message(role="assistant", content="Tool response", tool_calls=[mock_tool_call])
            ]
            mock_result = MagicMock()
            mock_result.final_state = mock_final_state
            return mock_result
    ...
    async def test_on_loop_detection_skips_tool_call(
        self, base_config, test_message, test_context, mock_model_provider, mock_jaf_run
    ):
...
class TestMultiAgentDelegation:
    ...
    def multi_agent_config(self, multi_agent, mock_model_provider):
        return RunnerConfig(agent=multi_agent, session_provider=AsyncMock(), max_llm_calls=3)

    async def test_delegation_to_multi_agent_runner(
        self, multi_agent_config, test_message, test_context, monkeypatch, mock_jaf_run
    ):
        """Verify that execute_agent delegates to execute_multi_agent for multi-agent setups."""

        # Mock the multi-agent executor
        mock_execute_multi_agent = AsyncMock(
