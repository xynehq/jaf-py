    ...
    async def validate_request(
        ...
        if user_id and self.rate_limiter:
            rate_result = await self.rate_limiter.check_rate_limit(
                user_id, self.config.rate_limit_window_seconds, self.config.max_requests_per_window
            )
...
def validate_api_key(api_key: str, expected_key: str) -> ValidationResult:
    ...
    if not api_key:
        return ValidationResult(
            is_valid=False, error_message="API key required", security_level=SecurityLevel.HIGH
        )

    # Use constant-time comparison to prevent timing attacks
    if not hmac.compare_digest(api_key, expected_key):
        return ValidationResult(
            is_valid=False, error_message="Invalid API key", security_level=SecurityLevel.HIGH
        )
...
def check_rate_limits(
    identifier: str, request_counts: Dict[str, List[float]], window_seconds: int, max_requests: int
) -> ValidationResult:
    """
