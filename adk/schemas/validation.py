...
def validate_schema(data: Any, schema: JsonSchema) -> ValidationResult:
    ...
    if data is None:
        if schema_type is None or "null" in (
    ...
    if schema_type == "string" and isinstance(data, str):
        ...
        return ValidationResult(
            success=False, errors=[f"Expected {schema_type}, got {type(data).__name__}"]
        )
...
def validate_string(data: str, schema: JsonSchema) -> ValidationResult:
    ...
    return ValidationResult(
        success=len(errors) == 0, errors=errors, data=data if len(errors) == 0 else None
    )
    ...
    for prop_name, prop_value in data.items():
        if prop_name in properties:
            ...
            if not add_prop_result.success:
                errors.extend(
                    [
                        f"Additional property '{prop_name}': {error}"
                        for error in add_prop_result.errors
                    ]
                )
