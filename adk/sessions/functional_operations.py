...
def add_assistant_message(content: str, tool_calls: Optional[List[Dict[str, Any]]] = None) -> SessionOperation:
    """
...
def sessions_equal(session1: ImmutableAdkSession, session2: ImmutableAdkSession) -> bool:
    ...
    return (
        ...
        session1.user_id == session2.user_id and
        session1.app_name == session2.app_name and
        session1.messages == session2.messages and
        session1.metadata == session2.metadata
    )


def get_session_diff(
    old_session: ImmutableAdkSession,
    new_session: ImmutableAdkSession
) -> Dict[str, Any]:
    """
...
def filter_sessions(predicate: SessionPredicate, sessions: List[ImmutableAdkSession]) -> List[ImmutableAdkSession]:
    """
...
def transform_session(operation: SessionOperation, session: ImmutableAdkSession) -> ImmutableAdkSession:
    """
