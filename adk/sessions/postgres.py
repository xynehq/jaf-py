...
class AdkPostgresSessionProvider(AdkSessionProvider):
    ...
    def __init__(
        self, config: AdkPostgresSessionConfig, connection_pool: Optional[AsyncPGPool] = None
    ):
    ...
    async def create_session(
        ...
        try:
            ...
            session_data = {"messages": self._serialize_messages(session.messages)}
            ...
            async with self.connection_pool.acquire() as connection:
                ...
                async with connection.transaction():
                    ...
                    user_session_count = await connection.fetchval(
                        f"SELECT COUNT(*) FROM {self.config.table_name} WHERE user_id = $1", user_id
                    )
