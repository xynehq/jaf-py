...
class TestToolCompositionProperties:
    ...
    def create_test_tool(self, name: str, delay_ms: int = 0, should_fail: bool = False):
        ...
        class TestTool:
            @property
            def schema(self):
                return ToolSchema(name=name, description=f"Test tool {name}", parameters=dict)
    ...
    async def test_retry_tool_properties(self, max_retries: int, backoff_factor: float):
        ...
        retry_tool = with_retry(
            failing_tool, max_retries=max_retries, backoff_factor=backoff_factor
        )
