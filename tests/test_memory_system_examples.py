...
def test_in_memory_provider():
    ...
    try:
        ...
        config = InMemoryConfig(max_conversations=1000, max_messages_per_conversation=1000)
...
async def test_memory_provider_protocol():
    ...
    try:
        ...
        result = await provider.append_messages(
            conversation_id="weather-chat-1", messages=new_messages
        )

        # Test get_recent_messages
        recent_result = await provider.get_recent_messages(
            conversation_id="weather-chat-1", limit=20
        )
...
def test_memory_config():
    ...
    try:
        ...
        memory_config = MemoryConfig(
            provider=provider, auto_store=True, max_messages=1000, ttl=86400
        )
...
async def test_manual_memory_operations():
    ...
    try:
        ...
        if isinstance(conv_result, Success) and conv_result.data:
            ...
            found_weather_question = any(
                "weather" in msg.content.lower() for msg in conversation.messages
            )
...
async def test_metadata_and_context():
    ...
    try:
        ...
        await provider.store_messages(
            conversation_id="support-ticket-456", messages=messages, metadata=metadata
        )
