...
class MockModelProvider:
    ...
    async def get_completion(self, state, agent, config):
        if self.call_count < len(self.tool_calls_sequence):
            ...
            self.call_count += 1
            return {"message": {"content": None, "tool_calls": tool_calls}}
        else:
            # Final response
            return {"message": {"content": "All timeout tests completed!", "tool_calls": None}}
...
async def test_all_timeout_scenarios():
    ...
    await run_timeout_scenario(
        "Tool-specific timeout (2s timeout, 5s operation)", tools, run_config_1, tool_calls_1
    )
    ...
    await run_timeout_scenario(
        "Tool-specific long timeout (10s timeout, 3s operation)", tools, run_config_1, tool_calls_2
    )
    ...
    await run_timeout_scenario(
        "RunConfig default timeout (3s timeout, 5s operation)", tools, run_config_3, tool_calls_3
    )
    ...
    await run_timeout_scenario(
        "Global default timeout (30s timeout, 2s operation)", tools, run_config_4, tool_calls_4
    )
        ...
        [
            {
                "id": "call_5",
                "type": "function",
                "function": {"name": "fast_tool", "arguments": '{"message": "Quick operation"}'},
            }
        ]
    ]

    await run_timeout_scenario("Fast tool (instant completion)", tools, run_config_1, tool_calls_5)
        ...
        [
            {
                "id": "call_6a",
                "type": "function",
                "function": {"name": "fast_tool", "arguments": '{"message": "First quick call"}'},
            },
            {
                "id": "call_6b",
                "type": "function",
            ...
            },
            {
                "id": "call_6c",
                "type": "function",
                "function": {"name": "fast_tool", "arguments": '{"message": "Another quick call"}'},
            },
        ]
    ]

    await run_timeout_scenario(
        "Multiple tools (1 fast, 1 timeout, 1 fast)", tools, run_config_1, tool_calls_6
    )
