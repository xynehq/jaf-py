...
async def test_a2a_deep_handoff():
    ...
    # 1. Create Agents
    math_tool_obj = create_a2a_tool(
        "math", "Performs calculations", MathArgs.model_json_schema(), math_tool
    )
    math_agent = create_a2a_agent(
        "MathAgent", "A mathematical agent", "You are a math agent.", [math_tool_obj]
    )

    handoff_tool_obj = create_a2a_tool(
        "handoff", "Handoffs to another agent", HandoffArgs.model_json_schema(), handoff_tool
    )
    triage_agent = create_a2a_agent(
        "TriageAgent", "A triage agent", "You are a triage agent.", [handoff_tool_obj]
    )
    ...
    respx.post("http://localhost:8080/a2a/agents/TriageAgent").mock(
                    ...
                    "tool_calls": [
                        {
                            ...
                            "function": {
                                ...
                                "arguments": json.dumps(
                                    {"agent_name": "MathAgent", "query": "What is 2 + 2?"}
                                ),
                            },
                        }
                    ]
    ...
    respx.post("http://localhost:8080/a2a/agents/MathAgent").mock(
        side_effect=[
            Response(
                200,
                json={
            ...
            Response(
                200, json={"jsonrpc": "2.0", "id": 3, "result": {"content": "The answer is 4."}}
            ),
        ]
    )
