...
def test_run_config_with_model_override():
    ...
    agent = Agent(name="TestAgent", instructions=lambda state: "Test instructions.", tools=[])
...
def test_structured_response_model():
    ...
    response = StructuredResponse(
        answer="Test answer", confidence=0.95, sources=["source1", "source2"]
    )
    ...
    class CachedModelProvider:
        ...
        def _create_cache_key(self, state, agent, config) -> str:
            ...
            key_data = {
                ...
                or (agent.model_config.name if agent.model_config else "default"),
...
def test_metrics_collecting_provider():
    ...
    class MetricsCollectingProvider:
        ...
        async def get_completion(self, state, agent, config) -> Dict[str, Any]:
            model_name = config.model_override or (
                agent.model_config.name if agent.model_config else "default"
            )
        ...
        def get_metrics_summary(self) -> Dict[str, Dict[str, Any]]:
            ...
            return {
                model: {
                    ...
                    "average_duration_ms": (
                        metrics.total_duration / metrics.successful_requests * 1000
                    )
                    if metrics.successful_requests > 0
                    else 0.0,
                    "recent_average_duration_ms": (
                        sum(metrics.recent_durations) / len(metrics.recent_durations) * 1000
                    )
...
async def test_async_provider_methods():
    ...
    agent = Agent(name="TestAgent", instructions=lambda s: "Test instructions", tools=[])

    # Create a mock provider for RunConfig
    mock_provider = Mock()
    config = RunConfig(agent_registry={"TestAgent": agent}, model_provider=mock_provider)
