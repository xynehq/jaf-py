...
class MathToolArgs(BaseModel):
    ...
    variables: Dict[str, Union[float, List[float]]] = Field(
        default={}, description="Dictionary of variables to use in the expression evaluation"
    )
    precision: Union[int, None] = Field(
        default=None, description="Number of decimal places to round the result to (optional)"
    )
...
async def test_math_tool_integration():
    ...
    config = RunConfig(
        agent_registry={"AdvancedMathAgent": agent}, model_provider=model_provider, max_turns=3
    )
