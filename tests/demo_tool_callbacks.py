...
class ToolCallbackDemo:
    ...
    async def on_start(self, context: RunContext, message: Message, session_state: Dict[str, Any]):
        self.log_event("LIFECYCLE_START", {
    ...
    async def on_complete(self, response: AgentResponse):
        self.log_event("LIFECYCLE_COMPLETE", {
    ...
    async def on_error(self, error: Exception, context: RunContext):
        self.log_event("LIFECYCLE_ERROR", {
    ...
    async def on_before_llm_call(self, agent: Agent, message: Message, session_state: Dict[str, Any]):
        self.log_event("LLM_BEFORE_CALL", {
    ...
    async def on_after_llm_call(self, response: Message, session_state: Dict[str, Any]):
        ...
        self.log_event("LLM_AFTER_CALL", {
            "response_role": response.role,
            "response_content": response.content[:100] + "..." if response.content and len(response.content) > 100 else response.content,
    ...
    async def on_before_tool_selection(self, available_tools: List[Tool], context_data: List[Any]):
        self.log_event("TOOL_BEFORE_SELECTION", {
    ...
    async def on_after_tool_execution(self, tool: Tool, result: Dict[str, Any]):
        self.log_event("TOOL_AFTER_EXECUTION", {
            "tool_name": tool.schema.name,
...
class ToolCallModelProvider:
    ...
    async def get_completion(self, state, agent, config) -> Dict[str, Any]:
        ...
        if "calculate" in last_message or "math" in last_message or any(op in last_message for op in ["add", "multiply", "divide", "subtract", "+", "*", "/", "-"]):
            ...
            if "15" in last_message and "25" in last_message:
                ...
                operation = "multiply" if "*" in last_message or "multiply" in last_message else "add"
                a, b = 7, 8
            else:
                operation = "add"
                a, b = 10, 5
