    ...
    async def get_completion(self, state, agent, config):
        ...
        if self.scenario == "tool_call":
                        ...
                        "tool_calls": [
                            {
                                ...
                                "function": {
                                    "name": "calculator",
                                    "arguments": json.dumps({"operation": "add", "a": 10, "b": 5}),
                                },
                            }
                        ],
                    }
            ...
            if self.call_count == 1:
                ...
                return {"message": {"content": "Error was handled properly.", "tool_calls": None}}
...
async def test_core_engine_basic_completion():
    ...
    config = RunConfig(
        agent_registry={"CoreTestAgent": agent}, model_provider=provider, max_turns=3
    )
...
async def test_core_engine_tool_execution():
    ...
    config = RunConfig(
        agent_registry={"ToolTestAgent": agent}, model_provider=provider, max_turns=3
    )
...
async def test_core_engine_agent_handoff():
    ...
    config = RunConfig(
        agent_registry={"SourceAgent": source_agent, "TargetAgent": target_agent},
...
async def test_core_engine_error_handling():
    ...
    config = RunConfig(
        agent_registry={"ErrorTestAgent": agent}, model_provider=provider, max_turns=3
    )
...
async def test_core_engine_context_preservation():
    ...
    config = RunConfig(
        agent_registry={"ContextTestAgent": agent}, model_provider=provider, max_turns=3
    )
                    ...
                    "tool_calls": [
                        {
                            ...
                            "function": {
                                "name": "calculator",
                                "arguments": json.dumps(
                                    {"operation": "add", "a": self.call_count, "b": 1}
                                ),
                            },
                        }
                    ],
