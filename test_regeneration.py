...
async def create_test_conversation(
    provider: MemoryProvider, conversation_id: str
) -> List[MessageId]:
    ...
    messages = [
        Message(
            ...
            content="What's the stock report for today?",
            message_id=msg_ids[0],
        ),
        Message(
            role=ContentRole.ASSISTANT, content="Today's market is up 2.5%", message_id=msg_ids[1]
        ),
        Message(role=ContentRole.USER, content="What was it yesterday?", message_id=msg_ids[2]),
        Message(
            role=ContentRole.ASSISTANT, content="Yesterday was down 1.2%", message_id=msg_ids[3]
        ),
        Message(role=ContentRole.USER, content="And the day before?", message_id=msg_ids[4]),
        Message(
            role=ContentRole.ASSISTANT, content="The day before was stable", message_id=msg_ids[5]
        ),
    ]
...
async def test_memory_provider_methods(
    provider: MemoryProvider, provider_name: str, results: TestResults
):
    ...
    try:
        ...
        results.record_test(
            f"{provider_name}: Create conversation", True, f"with {len(msg_ids)} messages"
        )
        ...
        if hasattr(truncate_result, "data"):
            ...
            results.record_test(
                f"{provider_name}: truncate_conversation_after",
                removed_count == 2,
                f"removed {removed_count} messages (expected 2)",
            )
        else:
            results.record_test(
                f"{provider_name}: truncate_conversation_after", False, "method failed"
            )
        ...
        msg_ids2 = await create_test_conversation(
            provider, conversation_id2
        )  # Use fresh conversation
        ...
        if hasattr(until_result, "data") and until_result.data:
            ...
            results.record_test(
                f"{provider_name}: get_conversation_until_message",
                len(until_messages) == 2,
                f"got {len(until_messages)} messages (expected 2)",
            )
        else:
            results.record_test(
                f"{provider_name}: get_conversation_until_message", False, "method failed"
            )
        ...
        if hasattr(truncate_result, "data"):
            removed_count = truncate_result.data
            results.record_test(
                f"{provider_name}: truncate non-existent message",
                removed_count == 0,
                "correctly handled missing message",
            )
        else:
            results.record_test(
                f"{provider_name}: truncate non-existent message",
                False,
                "failed to handle missing message",
            )
        ...
        results.record_test(
            f"{provider_name}: truncate non-existent conversation",
            has_error,
            "correctly returned error for missing conversation",
        )
...
async def test_regeneration_engine(results: TestResults):
    ...
    try:
        ...
        results.record_test(
            "Regeneration Engine: Basic regeneration", success, f"status: {result.outcome.status}"
        )
        ...
        results.record_test(
            "Regeneration Engine: Correct message count",
            final_count == expected_count,
            f"got {final_count}, expected {expected_count}",
        )

        # Check that subsequent messages were removed
        subsequent_found = any(
            msg.message_id in [msg_ids[3], msg_ids[4], msg_ids[5]]
            for msg in result.final_state.messages
        )
        results.record_test(
            "Regeneration Engine: Subsequent messages removed",
            not subsequent_found,
            f"subsequent messages found: {subsequent_found}",
        )
        ...
        results.record_test(
            "Regeneration Engine: History tracking",
            has_history,
...
async def test_edge_cases(results: TestResults):
    ...
    try:
        ...
        results.record_test(
            "Edge Cases: Store empty conversation",
            not hasattr(result, "error"),
            "stored empty conversation",
        )

        # Test 2: Regenerate from non-existent conversation
        fake_conv_id = "non-existent-conv"
        fake_msg_id = generate_message_id()
        truncate_result = await memory_provider.truncate_conversation_after(
            fake_conv_id, fake_msg_id
        )
        has_error = hasattr(truncate_result, "error")
        results.record_test(
            "Edge Cases: Non-existent conversation",
            has_error,
        ...
        both_successful = not hasattr(first_result, "error") and not hasattr(second_result, "error")
        results.record_test(
            "Edge Cases: Multiple regenerations",
            both_successful,
        ...
        first_msg_result = await memory_provider.truncate_conversation_after(
            first_conv_id, first_msg_ids[0]
        )
        if hasattr(first_msg_result, "data"):
            removed = first_msg_result.data
            results.record_test(
                "Edge Cases: Regenerate from first message",
                removed == 6,
                f"removed {removed} messages (expected 6)",
            )
        else:
        ...
        last_msg_result = await memory_provider.truncate_conversation_after(
            last_conv_id, last_msg_ids[5]
        )
        if hasattr(last_msg_result, "data"):
            removed = last_msg_result.data
            results.record_test(
                "Edge Cases: Regenerate from last message",
                removed == 1,
                f"removed {removed} messages (expected 1)",
            )
        else:
    ...
    try:
        ...
        try:
            ...
            if hasattr(postgres_result, "data"):
                ...
                results.record_test(
                    "PostgreSQL Provider", False, "Connection failed - skipping PostgreSQL tests"
                )
        except ImportError:
            results.record_test(
                "PostgreSQL Provider", False, "asyncpg not installed - skipping PostgreSQL tests"
            )
        except Exception as e:
            results.record_test(
                "PostgreSQL Provider", False, f"Connection error: {e} - skipping PostgreSQL tests"
            )
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if hasattr(redis_result, "data"):
                ...
                results.record_test(
                    "Redis Provider", False, "Connection failed - skipping Redis tests"
                )
        except ImportError:
            results.record_test(
                "Redis Provider", False, "redis not installed - skipping Redis tests"
            )
        except Exception as e:
            results.record_test(
                "Redis Provider", False, f"Connection error: {e} - skipping Redis tests"
            )
    except Exception as e:
