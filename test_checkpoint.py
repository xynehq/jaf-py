...
async def test_checkpoint_basic_functionality(
    ...
    try:
        ...
        results.record_test(
            f"{provider_name}: Basic checkpoint",
        ...
        results.record_test(
            f"{provider_name}: Correct message count",
        ...
        results.record_test(
            f"{provider_name}: Removed messages after checkpoint",
            not removed_found,
        ...
        results.record_test(
            f"{provider_name}: Checkpoint message preserved",
        ...
        results.record_test(
            f"{provider_name}: Messages before checkpoint preserved",
...
async def test_checkpoint_edge_cases(results: TestResults):
    ...
    try:
        ...
        results.record_test(
            "Edge Cases: Checkpoint after first message",
        ...
        results.record_test(
            "Edge Cases: Checkpoint after last message",
        ...
        checkpoint_request3 = CheckpointRequest(
            conversation_id=conv_id3, message_id=fake_msg_id, context={}
        )

        try:
            result3 = await checkpoint_conversation(checkpoint_request3, run_config3)
            results.record_test(
                "Edge Cases: Non-existent message", False, "should have raised ValueError"
            )
        except ValueError as e:
            results.record_test(
                "Edge Cases: Non-existent message",
                True,
        ...
        checkpoint_request4a = CheckpointRequest(
            conversation_id=conv_id4, message_id=msg_ids4[3], context={}
        )
        ...
        checkpoint_request4b = CheckpointRequest(
            conversation_id=conv_id4, message_id=msg_ids4[1], context={}
        )
        ...
        results.record_test(
            "Edge Cases: Multiple checkpoints",
...
async def test_checkpoint_history(results: TestResults):
    ...
    try:
        ...
        # Create a checkpoint
        checkpoint_request = CheckpointRequest(
            conversation_id=conv_id, message_id=msg_ids[2], context={"reason": "test checkpoint"}
        )
        ...
        results.record_test(
            "Checkpoint History: History exists",
            has_history,
...
async def test_checkpoint_vs_regeneration_difference(results: TestResults):
    ...
    try:
        ...
        checkpoint_request = CheckpointRequest(
            conversation_id=conv_id_chk, message_id=msg_ids_chk[2], context={}
        )
        ...
        checkpoint_includes_msg2 = msg_ids_chk[2] in [
            m.message_id for m in checkpoint_result.messages
        ]

        # Checkpoint should include message at index 2 (3 messages: 0, 1, 2)
        expected_checkpoint_count = 3
        results.record_test(
            "Difference: Checkpoint keeps target message",
        ...
        results.record_test(
            "Difference: Checkpoint behavior confirmed",
            True,
    ...
    try:
        ...
        try:
            ...
            redis_config = RedisConfig(
                host="localhost", port=6379, db=2, key_prefix="jaf_chk_test:"
            )
