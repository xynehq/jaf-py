...
from jaf import Agent, Tool, run_server, Message
...
async def create_filesystem_agents(filesystem_tools: List[FastMCPTool]) -> Dict[str, Agent]:
    ...
    def filesystem_instructions(state):
        ...
        tool_descriptions = "\n".join(
            [f"- {tool.schema.name}: {tool.schema.description}" for tool in filesystem_tools]
        )
...
Be helpful, safe, and informative in all filesystem operations!"""
    ...
    filesystem_agent = Agent(
        name="FilesystemAgent", instructions=filesystem_instructions, tools=filesystem_tools
    )
...
Keep operations simple and responses brief but informative."""
    ...
    return {"FilesystemAgent": filesystem_agent, "QuickFileAgent": quick_file_agent}
...
async def start_filesystem_server(mcp_url: Optional[str] = None):
    ...
    model_provider = make_litellm_provider(
        os.getenv("LITELLM_URL", "http://localhost:4000"), os.getenv("LITELLM_API_KEY")
    )
    ...
    try:
        ...
        print(
            '  -d \'{"messages":[{"role":"user","content":"List all files in my Desktop directory"}],"agentName":"FilesystemAgent","context":{"userId":"user_001","sessionId":"session_123"}}\''
        )
        ...
        print('  -H "Content-Type: application/json" \\')
        print(
            '  -d \'{"messages":[{"role":"user","content":"Create a file called hello.txt on my Desktop with the content: Hello from MCP filesystem agent!"}],"agentName":"FilesystemAgent","context":{"userId":"user_001","sessionId":"session_123"}}\''
        )
        ...
        print('  -H "Content-Type: application/json" \\')
        print(
            '  -d \'{"messages":[{"role":"user","content":"Read the contents of /Desktop/hello.txt"}],"agentName":"FilesystemAgent","context":{"userId":"user_001","sessionId":"session_123"}}\''
        )
        ...
        # Start the server
        await run_server(agents, run_config, host=host, port=port, cors=True)
