    ...
    async def execute(self, args: CalculateArgs, context: MyContext) -> Any:
            ...
            return ToolResponse.validation_error(
                "Invalid characters in expression. Only numbers, +, -, *, /, and () are allowed.",
                {
                    ...
                    "invalid_characters": "".join(
                        c for c in args.expression if c not in "0123456789+-*/(). "
                    ),
                },
            )
        ...
        try:
            ...
            return ToolResponse.error(
                ...
                {"expression": args.expression, "error": str(e)},
            )
...
def create_math_agent() -> Agent[MyContext, str]:
    ...
    return Agent(name="MathTutor", instructions=instructions, tools=[calculator_tool])
...
Important: You have access to the full conversation history through the messages. Use this history to provide contextual responses and remember what users have told you."""
    ...
    return Agent(name="ChatBot", instructions=instructions, tools=[greeting_tool])
...
def create_assistant_agent() -> Agent[MyContext, str]:
    ...
    return Agent(
        name="Assistant", instructions=instructions, tools=[calculator_tool, greeting_tool]
    )
...
async def start_server():
    ...
    try:
        ...
        if isinstance(result, Success):
            ...
            memory_config = {"provider": memory_provider, "auto_store": True, "max_messages": 100}
    ...
    try:
        ...
        print('     -H "Content-Type: application/json" \\')
        print(
            '     -d \'{"messages":[{"role":"user","content":"Hi, my name is Alice"}],"context":{"userId":"demo","permissions":["user"]}}\''
        )
        print("")
        ...
        print('     -H "Content-Type: application/json" \\')
        print(
            '     -d \'{"messages":[{"role":"user","content":"Calculate 25 + 17 and then greet me as Bob"}],"agent_name":"Assistant","context":{"userId":"demo","permissions":["user"]}}\''
        )
        print("")
        ...
        if memory_config:
            ...
            print('     -H "Content-Type: application/json" \\')
            print(
                '     -d \'{"messages":[{"role":"user","content":"Hello, I am starting a new conversation"}],"agent_name":"ChatBot","conversation_id":"my-conversation","context":{"userId":"demo","permissions":["user"]}}\''
            )
            print("")
            ...
            print('     -H "Content-Type: application/json" \\')
            print(
                '     -d \'{"messages":[{"role":"user","content":"Do you remember me?"}],"agent_name":"ChatBot","conversation_id":"my-conversation","context":{"userId":"demo","permissions":["user"]}}\''
            )
            print("")
