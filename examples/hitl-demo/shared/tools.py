...
class ListFilesArgs(BaseModel):
    ...
    directory: Optional[str] = Field(
        None, description="Directory to list (relative to working directory)"
    )
    ...
    async def execute(self, args: ListFilesArgs, context: FileSystemContext) -> str:
        try:
            ...
            for item in target_dir.iterdir():
                ...
                items.append({"name": item.name, "type": item_type, "path": str(relative_path)})
...
class ReadFileTool:
    ...
    def schema(self) -> ToolSchema[ReadFileArgs]:
        return ToolSchema(
            name="readFile", description="Read the contents of a file", parameters=ReadFileArgs
        )
