...
def create_redirect_tool() -> Tool[RedirectParams, Any]:
    ...
    class RedirectTool:
        ...
        async def execute(self, args: RedirectParams, context: Any) -> str:
            ...
            prev = (
                f" from {context.get('currentScreen', '')}" if context.get("currentScreen") else ""
            )
...
async def main():
    ...
    memory_config = MemoryConfig(provider=memory_provider, auto_store=True, max_messages=200)
