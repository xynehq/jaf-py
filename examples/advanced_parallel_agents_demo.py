...
def create_orchestrator_with_parallel_tools(specialists: Dict[str, Agent]) -> Agent:
    ...
    language_tool = create_language_specialists_tool(
        language_agents, tool_name="consult_language_specialists", timeout=300.0
    )
    ...
    try:
        ...
        def consensus_aggregator(results):
            ...
            if len(results) < 2:
                return {
                    "consensus": False,
                    "single_result": results[0] if results else "No results",
                }
            ...
            return {
                ...
                "all_results": results,
                "summary": f"Consensus reached on {len(set(common_themes))} themes"
                if common_themes
