...
async def summarize_large_context(
    state: RunState[SimpleContext], agent: Agent
    ...
    if estimated_tokens > 10000:
        ...
        if len(state.messages) > keep_recent + 1:
            ...
            summary_message = Message(role=ContentRole.ASSISTANT, content=summary_text)

            # Return state with summarized messages
            new_messages = [summary_message] + recent_messages
            print(
                f"[BEFORE LLM] Reduced from {len(state.messages)} to {len(new_messages)} messages"
            )
...
async def log_llm_response(
    state: RunState[SimpleContext], response: ModelCompletionResponse
) -> ModelCompletionResponse:
    """
    ...
    if response.message:
        content_preview = (
            ...
            if response.message.content and len(response.message.content) > 100
...
async def main():
    ...
    initial_state = RunState(
        ...
        messages=[Message(role=ContentRole.USER, content="What is the capital of France?")],
