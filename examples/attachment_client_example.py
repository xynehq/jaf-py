...
Be detailed but concise in your responses."""
    ...
    return Agent(
        ...
        model_config=ModelConfig(name="gpt-4o", temperature=0.7, max_tokens=500),
    )
...
async def test_attachments_with_agent():
    ...
    # Create run config
    config = RunConfig(
        agent_registry={"attachment-analyst": agent}, model_provider=model_provider, max_turns=3
    )
    ...
    try:
        text_attachment = make_file_attachment(
            data=SAMPLE_TEXT_B64, mime_type="text/plain", name="sample.txt"
        )

        message = Message(
            role="user", content="Please analyze this text file", attachments=[text_attachment]
        )
    ...
    try:
        image_attachment = make_image_attachment(
            data=TINY_PNG_B64, mime_type="image/png", name="sample-image.png"
        )
    ...
    try:
        image_attachment = make_image_attachment(
            data=TINY_PNG_B64, mime_type="image/png", name="tiny-pixel.png"
        )
    ...
    try:
        csv_attachment = make_file_attachment(
            data=SAMPLE_CSV_B64, mime_type="text/csv", name="data.csv"
        )
        ...
        try:
            response = await client.post(
                ...
                json={
                    ...
                    "messages": [
                        {"role": "user", "content": "Hello! Can you help me analyze attachments?"}
                    ],
                },
            )
