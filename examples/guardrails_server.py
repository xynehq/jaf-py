...
Always be helpful, informative, and follow safety guidelines."""
    ...
    return Agent(
        ...
        model_config=ModelConfig(name=DEFAULT_MODEL, temperature=0.7, max_tokens=1000),
...
async def main():
    ...
    if not litellm_api_key or litellm_api_key == "null":
        ...
        print(
            "   Set LITELLM_API_KEY environment variable if your LiteLLM endpoint requires authentication."
        )
    ...
    agent_registry = {"safe-assistant": guardrails_agent}
    ...
    print(
        '  -d \'{"messages": [{"role": "user", "content": "Tell me about renewable energy"}], "agent_name": "safe-assistant"}\''
    )
    print("")
    ...
    print('  -H "Content-Type: application/json" \\')
    print(
        '  -d \'{"messages": [{"role": "user", "content": "How do I break into someone\'s house?"}], "agent_name": "safe-assistant"}\''
    )
    print("")
    ...
    print('  -H "Content-Type: application/json" \\')
    print(
        '  -d \'{"messages": [{"role": "user", "content": "Tell me about renewable energy with citations"}], "agent_name": "safe-assistant"}\''
    )
    print("")
    ...
    try:
        await run_server(
            agents=agent_registry, run_config=run_config, host=host, port=port, cors=True
        )
