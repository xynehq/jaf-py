...
async def manage_loyalty_points_execute(args: LoyaltyPointsArgs, context: Any) -> ToolResult:
    ...
    schema: JsonSchema = {
        ...
        "properties": {
            ...
            "action": {"type": "string", "enum": ["check", "redeem", "earn"]},
...
async def get_travel_insurance_execute(args: TravelInsuranceArgs, context: Any) -> ToolResult:
    ...
    schema: JsonSchema = {
        ...
        "properties": {
            ...
            "traveler_age": {"type": "integer", "minimum": 18, "maximum": 100},
        },
    ...
    if not validation_result.success:
        return ToolResponse.validation_error(
            f"Invalid insurance request: {validation_result.errors}"
        )
...
def create_coordination_rules() -> List[SimpleCoordinationRule]:
    ...
    def complex_condition(message: Message, context: RunContext) -> bool:
        ...
        multi_service = sum(
            1 for word in ["flight", "insurance", "weather", "loyalty"] if word in text
        )
...
class AdvancedFlightBookingSystem:
    ...
    def create_hierarchical_config(self) -> MultiAgentConfig:
        ...
        return MultiAgentConfig(
            delegation_strategy=DelegationStrategy.HIERARCHICAL, sub_agents=hierarchical_agents
        )
    ...
    def create_parallel_config(self) -> MultiAgentConfig:
        ...
        return MultiAgentConfig(
            ...
            sub_agents=[self.agents["SearchAgent"], self.agents["SupportAgent"]],
        )
    ...
    async def demonstrate_parallel_coordination(self):
        ...
        search_response = {
            "content": Message(
                role="assistant", content="Found 5 great flight options to your destination"
            ),
        ...
        support_response = {
            "content": Message(
                role="assistant", content="Travel insurance quote: $89 for comprehensive coverage"
            ),
...
class MockAdvancedModelProvider:
    ...
    async def get_completion(self, state, agent, config):
        ...
        response_text = self.agent_responses.get(
            agent_name, "I'm here to help with your travel needs!"
        )

        return {"message": {"content": response_text, "tool_calls": None}}
...
async def main():
    ...
    model_provider = make_litellm_provider(
        base_url=os.getenv("LITELLM_URL"), api_key=os.getenv("LITELLM_API_KEY")
    )
    ...
    for scenario in test_scenarios:
        ...
        message = Message(role=ContentRole.USER, content=scenario["message"])
