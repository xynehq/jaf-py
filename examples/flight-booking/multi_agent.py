...
async def main():
    ...
    enhanced_search = with_logging(
        with_cache(
            with_retry(search_flights_tool.execute)
        )
    )
    ...
    invalid_data = FlightSearchArgs(origin="INVALID", destination="JFK", departure_date="2024-01-15")
