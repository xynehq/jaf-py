...
class FlightSearchArgs(BaseModel):
    ...
    class_preference: Optional[str] = Field(
        default="economy", description="Preferred class: economy, business, first"
    )
...
class BookFlightArgs(BaseModel):
    ...
    seat_preference: Optional[str] = Field(
        default="window", description="Seat preference: window, aisle, middle"
    )
...
MOCK_FLIGHTS = [
    ...
    Flight(
        ...
        datetime.now() + timedelta(days=1, hours=16),
        349.99,
        "United Airlines",
        32,
        "Airbus A320",
    ...
    Flight(
        ...
        "LAX",
        "LHR",
        datetime.now() + timedelta(days=2, hours=9),
...
async def search_flights_execute(args: FlightSearchArgs, context: Any) -> ToolResult:
    ...
    try:
        ...
        matching_flights = [
            ...
            if f.origin.lower() == args.origin.lower()
            and f.destination.lower() == args.destination.lower()
            and f.seats_available >= args.passengers
        ]
    ...
    try:
        ...
        if flight.seats_available < 1:
            return ToolResponse.validation_error(
                f"No seats available on flight {args.flight_number}."
            )
...
flight_booking_agent = Agent(
    ...
    model_config=ModelConfig(name="gemini-2.0-flash", temperature=0.1, max_tokens=1000),
)
