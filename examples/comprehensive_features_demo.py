        ...
        try:
            ...
            if any(op in args.expression for op in dangerous_operations):
                return ToolResponse.validation_error(
                    ...
                    details={
                        ...
                        "blocked_operations": dangerous_operations,
                    },
                )
            ...
            return ToolResponse.success(
                data={
                    ...
                    "functions_used": [
                        func for func in safe_dict.keys() if func in args.expression
                    ],
                },
                metadata={
                    "calculation_time": time.time(),
                    "complexity": "advanced"
                    if any(func in args.expression for func in safe_dict.keys())
                    else "basic",
                },
            )
...
class IntelligentSearchTool:
    ...
    async def execute(self, args: SearchArgs, context: ComprehensiveContext):
        ...
        self.search_history.append(
            {"query": args.query, "timestamp": time.time(), "user_id": context.user_id}
        )
...
def create_comprehensive_agent() -> Agent[ComprehensiveContext, str]:
    ...
    research_pipeline = create_tool_pipeline(
        cached_search, data_analysis, name="research_analysis_pipeline"
    )

    # Create parallel tools
    parallel_info = create_parallel_tools(
        cached_search, weather, name="parallel_info_gathering", combine_strategy="merge"
    )
...
async def demonstrate_comprehensive_analytics():
    ...
    messages = [
        ...
        Message(
            ...
            content="I'd be happy to help with data analysis. What specific dataset or analysis type are you interested in?",
        ),
        Message(role=ContentRole.USER, content="I have sales data and want to understand trends"),
        Message(
            role=ContentRole.ASSISTANT,
            content="Great! I can perform trend analysis on your sales data. Let me analyze the patterns for you.",
        ),
...
async def demonstrate_advanced_workflows():
    ...
    workflow_context = WorkflowContext(
        workflow_id="comprehensive_demo_workflow", user_context=context
    )

    # Create workflow steps
    search_step = AgentStep(
        "search_data", agent, "Search for information about machine learning trends"
    )

    analysis_step = AgentStep(
        "analyze_data", agent, "Analyze the search results and provide insights"
    )

    weather_step = AgentStep("get_weather", agent, "Get weather information for San Francisco")
    ...
    conditional_weather = ConditionalStep(
        "conditional_weather", should_include_weather, weather_step
    )

    # Create parallel step
    parallel_research = ParallelStep(
        "parallel_research", [search_step, weather_step], wait_for_all=True
    )
    ...
    loop_analysis = LoopStep(
        "iterative_analysis", analysis_step, should_continue_loop, max_iterations=3
    )

    # Build comprehensive workflow
    workflow = (
        create_workflow("comprehensive_demo", "Comprehensive Demo Workflow")
        ...
        .add_conditional_step(
            "conditional_analysis", lambda ctx: len(ctx.variables) > 0, analysis_step
        )
        .add_loop_step("iterative_refinement", analysis_step, should_continue_loop, 2)
        .with_step_callback(
            lambda result, ctx: print(
                f"  ✓ Step {result.step_id} completed in {result.execution_time_ms:.1f}ms"
            )
        )
        .with_completion_callback(
            lambda result: print(
                f"  🎉 Workflow completed with {result.success_rate:.1f}% success rate"
            )
        )
        .build()
    )
...
async def demonstrate_streaming_with_analytics():
    ...
    initial_state = RunState(
        ...
        messages=[
            Message(
                role=ContentRole.USER, content="Provide a comprehensive analysis with streaming"
            )
        ],
...
async def demonstrate_full_integration():
    ...
    async with monitor_performance() as monitor:
        ...
        calc_result = await calculator.execute(
            CalculateArgs(expression="sqrt(25) + sin(pi/2)"), context
        )
        monitor.record_tool_call()

        search_result = await search.execute(
            SearchArgs(query="JAF framework capabilities", max_results=3), context
        )
        monitor.record_tool_call()
        monitor.record_cache_miss()

        weather_result = await weather.execute(WeatherArgs(location="San Francisco"), context)
        monitor.record_tool_call()

        analysis_result = await data_analysis.execute(
            DataAnalysisArgs(dataset="user_engagement", analysis_type="trend"), context
        )
        monitor.record_tool_call()
