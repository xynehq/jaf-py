...
def process_data(
    text: str, count: int, enabled: bool = True, multiplier: float = 1.0, context=None
) -> str:
...
async def heavy_computation(dataset_size: int, algorithm: str = "standard", context=None) -> str:
...
async def database_query(query: str, table: str, limit: int = 100, context=None) -> str:
...
def demonstrate_tools():
    ...
    for tool in tools:
        ...
        print(
            f"Parameters: {tool.schema.parameters.__name__ if hasattr(tool.schema.parameters, '__name__') else 'Dynamic'}"
        )
        print("-" * 50)
