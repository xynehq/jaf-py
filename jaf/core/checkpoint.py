...
from .types import (
    RunState, RunConfig,
    CheckpointRequest, CheckpointContext,
    MessageId, Message,
    find_message_index, get_message_by_id,
...
async def checkpoint_conversation(
    ...
    conversation_result = await config.memory.provider.get_conversation(checkpoint_request.conversation_id)
    ...
    checkpointed_messages = original_messages[:checkpoint_index + 1]  # +1 to include the checkpoint message
    
    print(f"[JAF:CHECKPOINT] Checkpointed conversation to {len(checkpointed_messages)} messages")
    print(f"[JAF:CHECKPOINT] Original: {len(original_messages)}, Removed: {len(original_messages) - len(checkpointed_messages)}")
    ...
    print(f"[JAF:CHECKPOINT] Successfully checkpointed conversation {checkpoint_request.conversation_id}")
