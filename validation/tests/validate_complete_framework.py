...
class TestGreetingTool:
    ...
    def schema(self):
        return type(
            "ToolSchema",
            (),
            {"name": "greet", "description": "Generate greeting", "parameters": GreetArgs},
        )()
...
class MockModelProvider:
    ...
    async def get_completion(
        self, state: RunState[TestContext], agent: Agent, config: RunConfig
    ) -> Dict[str, Any]:
        if self.call_count >= len(self.responses):
            return {"message": {"content": "Default response", "tool_calls": None}}
...
async def test_core_functionality():
    ...
    agent = Agent(name="TestAgent", instructions=instructions, tools=[calc_tool, greet_tool])
    ...
    config = RunConfig(
        agent_registry={"TestAgent": agent}, model_provider=mock_provider, max_turns=5
    )
    ...
    mock_provider_with_tools = MockModelProvider(
        [
            ...
            {"message": {"content": "The result is 4.", "tool_calls": None}},
        ]
    )

    config_with_tools = RunConfig(
        agent_registry={"TestAgent": agent}, model_provider=mock_provider_with_tools, max_turns=5
    )
...
async def test_agent_handoffs():
    ...
    general_agent = Agent(
        name="GeneralAgent", instructions=general_instructions, tools=[TestGreetingTool()]
    )
    ...
    mock_provider = MockModelProvider(
        [
            ...
            {"message": {"content": "I can help with that!", "tool_calls": None}},
        ]
    )
    ...
    config = RunConfig(
        agent_registry={"MathAgent": math_agent, "GeneralAgent": general_agent},
...
async def test_error_handling():
        ...
        def schema(self):
            return type(
                ...
                (),
                {
                    ...
                    "parameters": type(
                        "LoopArgs", (BaseModel,), {"__annotations__": {"count": int}}
                    ),
                },
            )()
                    ...
                    "tool_calls": [
                        {
                            "id": "loop-1",
                            "type": "function",
                            "function": {"name": "loop", "arguments": '{"count": 1}'},
                        }
                    ],
                }
            },
                    ...
                    "tool_calls": [
                        {
                            "id": "loop-2",
                            "type": "function",
                            "function": {"name": "loop", "arguments": '{"count": 2}'},
                        }
                    ],
                }
            },
                    ...
                    "tool_calls": [
                        {
                            "id": "loop-3",
                            "type": "function",
                            "function": {"name": "loop", "arguments": '{"count": 3}'},
                        }
                    ],
                }
            },
                    ...
                    "tool_calls": [
                        {
                            "id": "loop-4",
                            "type": "function",
                            "function": {"name": "loop", "arguments": '{"count": 4}'},
                        }
                    ],
                }
            },
    ...
    agent = Agent(
        name="TestAgent", instructions=lambda state: "Keep using the loop tool", tools=[LoopTool()]
    )
    ...
    config = RunConfig(
        agent_registry={"TestAgent": agent}, model_provider=mock_provider, max_turns=50
    )
...
async def test_tracing_system():
    ...
    mock_provider = MockModelProvider([{"message": {"content": "Hello!", "tool_calls": None}}])

    agent = Agent(name="TestAgent", instructions=lambda state: "Simple agent", tools=[])
